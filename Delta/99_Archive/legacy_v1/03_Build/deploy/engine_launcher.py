...
_OUTPUT_DIR = _PROJECT_ROOT / "04_Test" / "result_output"
_ENV_FILE   = _PROJECT_ROOT / "04_Test" / ".env"
...
def run_pipeline(
    ...
    Args:
        blocks_gdf:     Pre-loaded block GeoDataFrame (Option B — from Patcher-Local via request body).
                        When provided, the DB block-loading step is skipped entirely.
    ...
    try:
        ...
        # Step 1 — load blocks (from parameter if provided, else from DB)
        ...
        # Cloud Function path (conn is None): records are returned as CSV to Patcher-Local,
        # which handles PostGIS insertion on the contractor's side.
        if conn is not None:
