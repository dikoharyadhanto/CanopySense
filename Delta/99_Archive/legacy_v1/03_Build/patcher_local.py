...
Usage:
  python3 patcher_local.py                  # scheduled: all blocks by afdeling
  python3 patcher_local.py --block-id 42   # upload: single block
...
import argparse, hashlib, json, logging, os, pathlib, sys, time, uuid
from itertools import groupby
import psycopg2, psycopg2.extras, requests
from dotenv import load_dotenv
logger = logging.getLogger(__name__)
_BUILD_DIR  = pathlib.Path(__file__).parent
_ENV_FILE   = _BUILD_DIR.parent / "04_Test" / ".env"
...
def _load_blocks(conn, block_id: int | None = None) -> list[dict]:
    sql = ("SELECT b.id,b.code,b.name,b.afdeling_id,ST_AsGeoJSON(b.geometry) "
           "FROM canopysense.blocks b ")
    params: tuple = (block_id,) if block_id is not None else ()
    sql += "WHERE b.id=%s" if block_id is not None else "ORDER BY b.afdeling_id,b.id"
...
def _log_write(conn, *, run_id, trigger_mode, afdeling_id, block_id,
               ...
               api_version=None, started_at_now=False) -> int:
    with conn.cursor() as cur:
        cur.execute(
            f"INSERT INTO {_LOG_TABLE} (run_id,trigger_mode,afdeling_id,block_id,batch_fingerprint,"
            "status,rows_inserted,error_detail,api_version,started_at) "
            "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,CASE WHEN %s THEN NOW() ELSE NULL END) RETURNING id",
...
def _presence_check_from_write(conn, write: dict, block_ids: list[int], schema: str) -> int:
    if not (pc := write.get("presence_check")): logger.warning("No presence_check metadata in write entry — skipping"); return 0
    c, d, days = pc["block_id_column"], pc["recency_column"], pc.get("recency_days", 14)
    with conn.cursor() as cur:
        cur.execute(f"SELECT COUNT(DISTINCT {c}) FROM {schema}.{write['table']} WHERE {c}=ANY(%s) AND {d}>=CURRENT_DATE-INTERVAL '{days} days'",(block_ids,))
        return cur.fetchone()[0]

def _call(url, api_key, contractor_id, fc, timeout) -> dict:
    resp = requests.post(url,
        headers={"X-API-Key":api_key,"Content-Type":"application/json","X-Contractor-Id":contractor_id},
        json={"api_version":"1.0","blocks":fc}, timeout=timeout)
    resp.raise_for_status(); return resp.json()

def _call_with_retry(url, api_key, contractor_id, fc, timeout, cb: list[int]) -> tuple[dict|None,str]:
    ...
    for attempt, wait_s in enumerate((*_BACKOFF, None), start=1):
        try:
            data = _call(url, api_key, contractor_id, fc, timeout)
...
def _run_batch(conn, run_id, trigger_mode, afdeling_id, block_id, blocks, url, api_key,
               contractor_id, timeout, cb, num, total, schema) -> str:
...
def _run_scheduled(conn, run_id, url, api_key, contractor_id, timeout, schema) -> int:
    mode = os.environ.get("BATCH_MODE","afdeling")
    all_blocks = _load_blocks(conn)
    if not all_blocks:
        logger.warning("[WARN]  No blocks found in canopysense.blocks. Nothing to do."); return 0
    batches = _group_batches(all_blocks, mode)
    logger.info("[INFO]  Run started — mode: scheduled | run_id: %s", run_id[:8])
    ...
    for i, (aid, rinfo, blks) in enumerate(ordered, 1):
        if not blks:
            ...
            _log_write(conn,run_id=run_id,trigger_mode="scheduled",afdeling_id=aid,block_id=None,
                       batch_fingerprint=None,status="SKIPPED"); counts["SKIPPED"]+=1; continue
        ...
        st = _run_batch(conn,run_id,"scheduled",aid,None,blks,url,api_key,contractor_id,timeout,cb,i,len(ordered),schema)
...
def _run_upload(conn, run_id, block_id, url, api_key, contractor_id, timeout, schema) -> int:
    logger.info("[INFO]  Run started — mode: upload | block_id=%d | run_id: %s", block_id, run_id[:8])
    blks = _load_blocks(conn, block_id=block_id)
    if not blks:
        logger.info("[INFO]  [SKIPPED] block_id=%d not found in canopysense.blocks", block_id)
        _log_write(conn,run_id=run_id,trigger_mode="upload",afdeling_id=None,
                   block_id=block_id,batch_fingerprint=None,status="SKIPPED"); return 0
    st = _run_batch(conn,run_id,"upload",None,blks[0]["block_id"],blks,url,api_key,contractor_id,timeout,[0],1,1,schema)
    return 0 if st in ("FULL_SUCCESS","PARTIAL_SUCCESS") else 1

def main() -> int:
    parser = argparse.ArgumentParser(description="CanopySense Patcher-Local v0.9")
    ...
    try:
        if args.block_id is not None:
            return _run_upload(conn,run_id,args.block_id,url,api_key,contractor_id,timeout,schema)
