...
Tabel satellite_data Menyimpan data pra-pemrosesan metrik satelit (seperti Sentinel-2 atau Landsat-8/9) per akuisisi. Tabel ini merekam tingkat tutupan awan (cloud cover) dan kelima indeks vegetasi utama yang menjadi fitur (feature engineering) dalam pemodelan.
...
Tabel predictions Menyimpan keluaran (output) prediksi Green Canopy Cover (%) dari model algoritma Random Forest atau XGBoost. Informasi tingkat interval kepercayaan (confidence interval) juga disimpan di sini
...
CREATE TABLE predictions (
    ...
    gcc_predicted NUMERIC(5, 2),

    gcc_confidence NUMERIC(5, 2),
...
Tabel anomalies menyimpan catatan ketika sistem mendeteksi deviasi/anomali kesehatan kanopi (berdasarkan dekomposisi nilai residual time-series). Status anomali dibagi berdasarkan keparahannya (severity)
...
Tanggal anomali terdeteksi

anomaly_type

VARCHAR(50)

Tipe anomali ('Negative', 'Positive')

severity

VARCHAR(20)

Tingkat keparahan ('Waspada', 'Kritis')

residual_value

NUMERIC(6, 2)

Nilai residual dari dekomposisi STL

status

VARCHAR(20)

DEFAULT 'Open', Status inspeksi ('Open', 'Inspected', 'Resolved')
...
Tabel field_inspections mencatat hasil penugasan pemeriksaan lapangan oleh petugas lapangan saat merespons sebuah anomali. Tabel ini berupa penyimpanan JSONB pada PostgreSQL untuk menyimpan format array berisi tautan foto.
...
PRIMARY KEY & REFERENCES estates(id)
...
Komposit user_id dan estate_id guna mencegah redundansi
...
Bagian ini menyajikan representasi visual dari arsitektur basis data Canopy Sense, yang mengilustrasikan keterkaitan antar entitas melalui relasi kunci (Primary Key, Foreign Key) serta kardinalitas sistem sesuai dengan implementasi skema PostGIS 4.1 Model Fisik Relasional.

![placeholder](https://markdowntoword.io/placeholder.png)

Arsitektur data dalam ekosistem Canopy Sense dibuat berbasis hierarki spasial (estates > afdelings > blocks) yang berfungsi sebagai jangkar bagi integrasi seluruh dataset analitik time-series serta data operasional. Adapun struktur relasi kunci utama didefinisikan sebagai berikut:

- Relasi One-to-Many diterapkan pada hierarki tabel spasial secara berjenjang, mulai dari entitas estates ke afdelings, hingga afdelings ke blocks.
- Tabel atribut analitik meliputi ground_truth, satellite_data, dan predictions yang memiliki relasi One-to-Many dengan tabel blocks melalui referensi block_id.
- Entitas anomalies terhubung secara One-to-Many terhadap unit wilayah pada tabel blocks.
- Tabel operasional alerts dan field_inspections dikaitkan melalui relasi One-to-Many dengan tabel anomalies.
- Tabel users berperan sebagai subjek dalam relasi One-to-Many terhadap alerts (sebagai identitas penerima) serta field_inspections (sebagai petugas inspeksi lapangan).
- Keterkaitan antar pengguna dan wilayah kebun dimodelkan sebagai relasi Many-to-Many (N-N) melalui tabel asosiatif user_estate_roles.
